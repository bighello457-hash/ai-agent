...
/**
 * 处理工具调用的基础代理类，具体实现了think方法和act方法，可以用作创建实例的父类
 */
...
public class ToolCallAgent extends ReActAgent{
    ...
    public ToolCallAgent(ToolCallback[] availableTools){
        ...
        //禁用内置的spring ai 内置的工具调用机制，自己维护选项和消息上下文
    ...
    public String act() {
        ...
        AssistantMessage.ToolCall toolCall = toolCallList.get(0);

        if ("askHuman".equals(toolCall.name())) {
//            String question = extractQuestion(toolCall.arguments());

            System.out.println("[FxsManus 需要用户补充信息] " );

            String userAnswer = ConsoleInputHolder.SCANNER.nextLine();

            getMessageList().add(new UserMessage("用户回答：" + userAnswer));
